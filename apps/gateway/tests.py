...
ROUTES = [
    {
        "pattern": "/api/v1/my-example/{id}/example_1",
        "methods": ("GET",),
        "requires_auth": True,
        "target_path": "",
        "is_active": True,
...
def test_get_backend_url_with_target_path():
    ...
    route = URIPatternData(
        ...
        methods=("GET",),
        requires_auth=True,
        target_path="https://example-test.com/",
    )
    url = get_backend_url(path=path, route=route)
    assert expected_url == url
...
async def test_cache_refreshed_on_upstream_change():
    ...
    upstream = await sync_to_async(Upstream.objects.create)(
        name="Test Upstream", base_url="http://old-url.com"
    )
    await sync_to_async(URIPattern.objects.create)(
        pattern="/test", methods="GET", upstream=upstream, is_active=True
    )
